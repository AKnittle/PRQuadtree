...
public class Lewis
{
    ...
    public void checkTreeInitialization()
    {
        ...
        {
            System.out
                .println("Error writing notes to log file in Lewis.checkTreeInitialization().");
        }
    }
        ...
        throws IOException
    {
        ...
        {
            System.out
                .println("Error writing notes to log file in Lewis.checkInsertion().");
        }
        String Notes = new String();
        prQuadTree<Point> Tree = new prQuadTree<Point>(xMin, xMax, yMin, yMax);
        ...
        {
            Notes +=
                writePoints(0) + "   Inserting value: "
                    + data.elementAt(dataIdx) + "\n";
            ...
            catch (Exception e)
            {
                Notes +=
                    writePoints(0)
                        + "   Caught an exception while inserting value:\n";
            ...
            try
            {
                ...
                {
                    Notes +=
                        writePoints(0) + "   Error: insert() returned false.\n";
                }
                else
                {
        ...
        try
        {
            ...
            Notes +=
                writePoints(0) + "   Now trying to insert a duplicate entry: "
                    + data.elementAt(dataIdx) + "\n";
            boolean success;
            try
            {
        ...
        {
            Notes +=
                writePoints(0) + "   Caught an exception during insertion.\n";
            Notes += writePoints(0) + "     " + e.getMessage() + "\n";
        }
        ...
        throws IOException
    {
        ...
        {
            System.out
                .println("Error writing notes to log file in Lewis.checkDeletion().");
        }
        String Notes = new String();
        prQuadTree<Point> Tree = new prQuadTree<Point>(xMin, xMax, yMin, yMax);
        ...
        Notes +=
            writePoints(0)
                + "     Building a tree for testing...(if checkInsertion() failed this should fail also)\n";
        ...
        catch (Exception e)
        {
            Notes +=
                writePoints(0)
                    + "   Caught an exception while building tree:\n";
            Notes += writePoints(0) + "     " + e.getMessage() + "\n";
            ...
            for (int dataIdx = 0; dataIdx < data.size(); dataIdx++)
            {
                ...
                try
                {
                    if (!success)
                    {
                        Notes +=
                            writePoints(0)
                                + "   Bummer: delete() returned false.\n";
                    }
                    else
                    {
                        Notes +=
                            writePoints(0) + "   delete() returned true.\n";
                    }
                    Notes += writePoints(0) + "   Resulting tree:\n";
                    Log.write(Notes);
                    Notes = "";
        ...
        catch (Exception e)
        {
            Notes +=
                writePoints(0)
                    + "   Caught an exception while performing deletion:\n";
            Notes += writePoints(0) + "     " + e.getMessage() + "\n";
            Notes += writePoints(0) + "   Aborting deletion test.\n";
            Log.write(Notes);
            Log.close();
        ...
        throws IOException
    {
        ...
        {
            System.out
                .println("Error writing notes to log file in Lewis.checkRegionSearch().");
        }
        String Notes = new String();
        prQuadTree<Point> Tree = new prQuadTree<Point>(xMin, xMax, yMin, yMax);
        ...
        try
        {
            ...
            runRegionSearch(Tree, xCenter - xDelta, xCenter + xDelta, yCenter
                - yDelta, yCenter + yDelta, Log);
            ...
            if (yC - yDelta < yMin)
                ...
                yDelta = yMax - yC;
            runRegionSearch(Tree, xC - xDelta, xC + xDelta, yC - yDelta, yC
                + yDelta, Log);
        ...
        catch (Exception e)
        {
            ...
            Notes +=
                writePoints(0)
                    + "   Caught an exception while testing region search.\n";
            Notes += writePoints(0) + "     " + e.getMessage() + "\n";
            Notes += writePoints(0) + "   Aborting region search test.\n";
            Log.write(Notes);
            Log.close();
        ...
        throws IOException
    {
        ...
        {
            Notes +=
                writePoints(0) + "Searching region: " + "(" + xLo + ", " + xHi
                    + ", " + yLo + ", " + yHi + ")" + "\n";

            Vector<Point> results = Tree.find(xLo, xHi, yLo, yHi);
            Notes += writePoints(0) + "Results:\n";
            if (results.isEmpty())
            {
                Notes +=
                    writePoints(20)
                        + "  no data points found within that region.\n";
            }
            else
            {
                // sortPoints(results);
    ...
    private void sortPoints(Vector<Point> points)
    {
        ...
        int ptsPerDataItem)
    {
        ...
        try
        {
            ...
            if (sRoot.getClass().equals(Leaf.getClass()))
            {
                ...
                {
                    Out.write(writePoints(ptsPerDataItem) + " " + Padding
                        + p.Elements.get(pos) + "\n");
                }
            }
